...
import 'package:efood_table_booking/data/model/response/error_response.dart';
...
class ApiClient extends GetxService {
  ...
  static const String noInternetMessage = 'La conexión al servidor API falló debido a la conexión a Internet';
  ...
  Future<Response> getDataPuntosAzules(String uri, {Map<String, dynamic>? query, Map<String, String>? headers}) async {
    try {
      debugPrint('====> API Call: $uri\n header: $_mainHeaders \n Token: $token');
      updateHeaderCoffeSac();
      http.Response response0 = await http.get(
        Uri.parse(sharedPreferences.getString(AppConstants.nombreBaseUrlPuntoAzul)!+uri),
        headers: headers ?? _mainHeaders,
      ).timeout(Duration(seconds: timeoutInSeconds));
      Response response = handleResponse(response0);
      debugPrint('====> API Response: [${response.statusCode}] $uri\n${response.body}');
  ...
  Future<Response> postDataPuntosAzules(String? uri, dynamic body, {Map<String, String>? headers}) async {
    try {
      updateHeaderCoffeSac();
      debugPrint('====> API Call: $uri\nToken: $token');
      debugPrint('====> API Body: $body');
      String? url = sharedPreferences.getString(AppConstants.nombreBaseUrlCoffeeSacSoftBank)!+
          sharedPreferences.getString(AppConstants.nombreCoffeeSacAmbiente)!+
          uri!;
      ...
      //debugPrint('====> RESPONSE COFFEE SAC:'+response.statusCode.toString()+" - "+errorBody["excepcion"]);
      return handleResponse(response);
    } catch (e) {
      debugPrint('====> ERROR COFFEE SAC: $e');
      return const Response(statusCode: 1, statusText: noInternetMessage);
    }
  }
  ...
  Future<Response> postProspectoGenerico(String? uri, dynamic body, {Map<String, String>? headers}) async {
    try {
      updateHeaderCoffeSac();
      debugPrint('====> API Call: $uri\nToken: $token');
      //debugPrint('====> API Body: $body');
