...
import 'package:efood_table_booking/view/base/stock_tag_view.dart';
...
import 'package:efood_table_booking/view/screens/product_image/image_screen.dart';
...
class ProductBottomSheet extends StatefulWidget {
  ...
  final int? cartIndex;
  const ProductBottomSheet({super.key, required this.product, this.cart, this.cartIndex});
          ...
          Container(
            ...
            decoration: BoxDecoration(
              ...
              borderRadius: BorderRadius.only(
                ...
                bottomLeft: isTab ? Radius.circular(Dimensions.radiusExtraLarge) : Radius.zero,
                bottomRight: isTab ? Radius.circular(Dimensions.radiusExtraLarge) : Radius.zero,
              ),
            ),
            child: GetBuilder<CartController>(builder: (cartController) {
                return GetBuilder<ProductController>(builder: (productController) {
                  ...
                  if(widget.product.branchProduct != null && widget.product.branchProduct!.isAvailable!) {
                    ...
                    discountType = widget.product.branchProduct?.discountType ?? 'percent';
                    ...
                    if(variationList[index].variationValues != null) {
                      for(int i=0; i < variationList[index].variationValues!.length; i++) {
                        if(productController.selectedVariations[index][i]) {
                          variationPrice += variationList[index].variationValues![i].optionPrice ?? 0;
                        }
                      }
                    }
                  ...
                  double priceWithDiscount = PriceConverter.convertWithDiscount(
                    price, discount, discountType.toString(),
                  );

                  double addonsCost = 0;
                  double addonsTax = 0;
                  List<AddOn> addOnIdList = [];
                  int addonLen = widget.product.addOns != null ? widget.product.addOns!.length : 0;

                  for (int index = 0; index < addonLen; index++) {
                    if (productController.addOnActiveList[index]) {
                      double addonItemPrice = widget.product.addOns![index].price! * productController.addOnQtyList[index];
                      addonsCost = addonsCost + (addonItemPrice);
                      addonsTax = addonsTax + (addonItemPrice - PriceConverter.convertWithDiscount((addonItemPrice), widget.product.addOns![index].tax ?? 0, 'percent'));
                      ...
                      Flexible(
                        child: SingleChildScrollView(
                          child: Column(
                            ...
                            children: [
                              Padding(
                                padding: EdgeInsets.all(isTab ? Dimensions.paddingSizeExtraLarge : Dimensions.paddingSizeLarge),
                                  ...
                                  Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                                    ...
                                    Row(mainAxisAlignment: MainAxisAlignment.center, crossAxisAlignment: CrossAxisAlignment.start, children: [
                                      ...
                                      (widget.product.image != null && widget.product.image!.isNotEmpty) ? Stack(children: [
                                        InkWell(
                                           onTap:()=> Get.to(()=> ProductImageScreen(product: widget.product), transition: Transition.leftToRight, duration: const Duration(milliseconds: 300)),
                                          child: ClipRRect(
                                            borderRadius: BorderRadius.circular(Dimensions.radiusSmall),
                                            child: CustomImage(
                                              image: '${Get.find<SplashController>().configModel?.baseUrls?.productImageUrl}/${widget.product.image ?? ''}',
                                              width: ResponsiveHelper.isSmallTab() ? 100 :  isTab ? 170 : 100,
                                              height: ResponsiveHelper.isSmallTab() ? 100 : isTab ? 170 : 100,
                                              fit: BoxFit.cover,
                                            ),
                                          ),
                                        ),
                                       ...
                                       if(discount > 0)
                                         PriceStackTag(value: PriceConverter.percentageCalculation(
                                          price.toString(), discount.toString(), discountType,
                                        )),
                                    ...
                                    variationList.isNotEmpty ? ListView.builder(
                                      ...
                                      itemBuilder: (context, index) {
                                        return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                                          ...
                                          Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                                           ...
                                           if(index == 0) ButtonWidget(cartIndex: widget.cartIndex ?? -1, product: widget.product),
                                          ]),
                                         ...
                                         if(variationList[index].variationValues != null) ListView.builder(
                                            shrinkWrap: true,
                                            physics: const NeverScrollableScrollPhysics(),
                                            padding: EdgeInsets.zero,
                                              ...
                                              return InkWell(
                                                ...
                                                child: Row(children: [
                                                  ...
                                                  Row(crossAxisAlignment: CrossAxisAlignment.center, children: [
                                                    ...
                                                    Text(
                                                      variationList[index].variationValues![i].level != null
                                      ...
                                      ButtonWidget(cartIndex: widget.cartIndex ?? -1, product: widget.product),
                                    ]),

                                    // Variation

                                   SizedBox(height: widget.product.choiceOptions!.isNotEmpty ? Dimensions.paddingSizeDefault : 0),

                                    // Addons
                                    widget.product.addOns != null  && widget.product.addOns!.isNotEmpty ? Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                                      Row(
                                        children: [
                                              ...
                                              child: Container(
                                                ...
                                                child: Column(children: [
                                                  ...
                                                  productController.addOnActiveList[index] ? Container(
                                                    ...
                                                    decoration: BoxDecoration(
                                                      borderRadius: BorderRadius.circular(Dimensions.radiusSmall), color: Theme.of(context).canvasColor,
                                                    ),
                                                    child: Row(children: [
                                                      Expanded(
                                                        child: InkWell(
                            ...
                            children: [
                              isTab ? Flexible(
                                ...
                                child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                                  ...
                                  Text(PriceConverter.convertPrice(priceWithAddonsVariation), style: robotoBold.copyWith(fontSize: Dimensions.fontSizeExtraLarge)),
                                  SizedBox(width: Dimensions.paddingSizeExtraSmall),
                              ...
                              Flexible(
                                ...
                                child: CustomButton(
                                  ...
                                  (widget.cart != null) ? 'update_in_cart'.tr : 'add_to_cart'.tr : 'not_available'.tr,
                                  onPressed: widget.cart == null && !productController.checkStock(widget.product, quantity: cartController.getCartProductQuantityCount(widget.product))  ? null : isAvailable ? () {
                                    ...
                                    if(variationList.isNotEmpty){
                                      for(int index=0; index<variationList.length; index++) {
                                        if(!variationList[index].isMultiSelect! && variationList[index].isRequired!
                                          ...
                                          showCustomSnackBar('${'choose_a_variation_from'.tr} ${variationList[index].name}', isToast: true, isError: true);
                                          return;
                                        }else if(variationList[index].isMultiSelect! && (variationList[index].isRequired!
                                            || productController.selectedVariations[index].contains(true)) && variationList[index].min!
                                            > productController.selectedVariationLength(productController.selectedVariations, index)) {
                                          showCustomSnackBar('${'you_need_to_select_minimum'.tr} ${variationList[index].min} '
                                              '${'to_maximum'.tr} ${variationList[index].max} ${'options_from'.tr} ${variationList[index].name} ${'variation'.tr}', isError: true, isToast: true);
                                          return;
                                        }
                                      }
                                    }
                                    ...
                                    cart.CartModel cartModel = CartModel(
                                      ...
                                      variation:  [],
                                      discountAmount: priceWithVariation - PriceConverter.convertWithDiscount(
                                        priceWithVariation, discount, discountType,
                                      ),
                                      quantity:  productController.quantity,
                                      taxAmount: (priceWithVariation  - PriceConverter.convertWithDiscount( priceWithVariation, widget.product.tax!, widget.product.taxType!) + addonsTax),
                                    ...
                                    cartController.addToCart(cartModel, widget.cart != null ? widget.cartIndex! : productController.cartIndex);

                                    showCustomSnackBar('Artículo agregado al carrito', isError: false, isToast: true);
          ...
          child: Align(
            ...
            child: Padding(
              ...
              child: CustomRoundedButton(
                widget: const Icon(Icons.close_outlined, size: 15), image: '', onTap: () => Get.back(),
              ),
            ),
          ),
        ),
  ...
  Widget build(BuildContext context) {
    return GetBuilder<SplashController>(
      builder: (splashController) {
        return splashController.configModel!.isVegNonVegActive! ? Container(
          ...
          child: product.productType == null ? const SizedBox() :
...
class ButtonWidget extends StatelessWidget {
  const ButtonWidget({
    super.key,
    required int cartIndex, this.product,
        ...
        return Row(children: [
           ...
           QuantityButton(
             ...
             onTap: (){
               ...
               int quantity =  cartController.getCartProductQuantityCount(_cartIndex != -1 ? cartController.cartList[_cartIndex].product : product);

               if(productController.checkStock(
                 _cartIndex != -1 ? cartController.cartList[_cartIndex].product : product,
