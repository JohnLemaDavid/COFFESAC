...
class ProductController extends GetxController implements GetxService {
  ...
  Future<void> getProductList(bool reload, bool notify, {String? productType, String? searchPattern, String? categoryId,   int offset = 1}) async {
    ...
    if(_productList == null || reload || !_offsetList.contains(offset)) {
      ...
      Response response = await productRepo.getProductList(offset, productType, searchPattern, categoryId);
  ...
  int setExistInCart(Product product, {bool notify = true}) {
    ...
    if(_cartIndex != -1) {
      ...
      _addOnQtyList = [];
      List<int> addOnIdList = [];
      Get.find<CartController>().cartList[_cartIndex].addOnIds?.forEach((addOnId) => addOnIdList.add(addOnId.id!));
      product.addOns?.forEach((addOn) {
        if(addOnIdList.contains(addOn.id)) {
          _addOnActiveList.add(true);
  ...
  void setCartVariationIndex(int index, int i, Product product, bool isMultiSelect) {
    if(!isMultiSelect) {
      ...
      if(!_selectedVariations[index][i] && selectedVariationLength(_selectedVariations, index) >= product.variations![index].max!) {
        showCustomSnackBar('${'maximum_variation_for'.tr} ${product.variations?[index].name} ${'is'.tr} ${product.variations?[index].max}', isToast: true);
      }else {
  ...
  bool checkStock(Product? product, {int? quantity}){
    int? stock;
    if(product != null && product.branchProduct?.stockType != 'unlimited' && product.branchProduct?.stock != null && product.branchProduct?.soldQuantity != null){
